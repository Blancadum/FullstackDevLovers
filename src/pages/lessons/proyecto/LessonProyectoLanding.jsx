      ...
      {breadcrumbs && breadcrumbs.length > 0 && (
        <div className="proyecto-landing-breadcrumbs">
          {breadcrumbs.map((crumb, index) => (
            <span key={index} className="proyecto-landing-breadcrumb-item">
              {index > 0 && <span className="proyecto-landing-breadcrumb-separator">›</span>}
              <a
                href={crumb.url || '#'}
                className={`proyecto-landing-breadcrumb-link ${index === breadcrumbs.length - 1 ? 'active' : ''}`}
              >
                {crumb.label}
              </a>
            </span>
