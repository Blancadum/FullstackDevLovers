...
export function LessonDockerOptimizacion() {
  ...
  const concepts = [
    { title: 'Alpine', definition: 'Distribución Linux ultra-ligera para contenedores', example: 'node:18-alpine (170MB vs 900MB)' },
    { title: 'Layer Caching', definition: 'Reutiliza capas no modificadas', example: 'COPY package.json antes que código' },
    { title: 'Limpieza', definition: 'Elimina archivos temporales en construcción', example: 'RUN apt-get clean && rm -rf /var/lib/apt/lists/*' },
    { title: '.dockerignore', definition: 'No copia archivos innecesarios', example: 'node_modules, .git, .env' },
    { title: 'Seguridad', definition: 'No ejecutar como root, actualizar librerías', example: 'USER appuser' },
    { title: 'Monitoreo', definition: 'Limitar recursos, health checks', example: '-m 512m, HEALTHCHECK' }
  ];
