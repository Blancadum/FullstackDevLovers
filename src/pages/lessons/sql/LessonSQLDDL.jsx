...
export function LessonSQLDDL() {
  ...
  const sections = [
    {
      ...
      content: (
        <p>
          <strong>DDL (Data Definition Language)</strong> son los comandos <strong>SQL</strong> que definen la estructura y esquema de la <strong>base de datos</strong>.
    ...
    },
    {
      ...
      content: (
        <>
          <p>El comando <strong>CREATE TABLE</strong> define una nueva <strong>tabla</strong> con sus <strong>columnas</strong> y <strong>restricciones</strong>:</p>
          <CodeBlock
            language="sql"
  ...
  fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    ...
    },
    {
      ...
      content: (
        <>
          <p>Las <strong>restricciones</strong> aseguran la integridad y validez de los datos:</p>
          <Table
            headers={['Restricción', 'Descripción', 'Ejemplo']}
            rows={[
              ['<strong>PRIMARY KEY</strong>', 'Identifica cada fila únicamente', 'id INT PRIMARY KEY'],
              ['<strong>FOREIGN KEY</strong>', 'Referencia a otra tabla', 'usuario_id INT FOREIGN KEY REFERENCES usuarios(id)'],
              ['<strong>NOT NULL</strong>', 'El campo es obligatorio', 'nombre VARCHAR(50) NOT NULL'],
              ['<strong>UNIQUE</strong>', 'Valores no repetidos', 'email VARCHAR(100) UNIQUE'],
              ['<strong>CHECK</strong>', 'Valida una condición', 'edad INT CHECK (edad >= 18)'],
              ['<strong>DEFAULT</strong>', 'Valor por defecto', 'estado VARCHAR(20) DEFAULT "activo"']
            ]}
          />
        </>
      )
    },
    {
      ...
      content: (
        <>
          <p><strong>ALTER TABLE</strong> permite cambiar la estructura de una <strong>tabla</strong> existente:</p>
          <CodeBlock
            language="sql"
...
DROP TABLE IF EXISTS usuarios;`}
          ...
          <InfoBox type="warning" title="Diferencia importante">
            <strong>DROP</strong> elimina la <strong>tabla</strong> y su estructura. <strong>TRUNCATE</strong> solo vacía los datos pero la estructura permanece.
            <strong>TRUNCATE</strong> es más rápido pero no se puede deshacer fácilmente.
          </InfoBox>
        </>
      )
    },
    {
      ...
      content: (
        <>
          <p>Los <strong>índices</strong> mejoran la velocidad de búsqueda:</p>
          <CodeBlock
            language="sql"
