  ...
  const sections = [
    {
      ...
      content: (
        <p>
          Las <strong>consultas avanzadas</strong> combinan múltiples técnicas para extraer datos complejos y
    ...
    },
    {
      ...
      content: (
        <>
          <p>Una <strong>subconsulta</strong> es una consulta dentro de otra:</p>
          <CodeBlock
            language="sql"
...
FROM usuarios u;`}
          />
          <InfoBox type="info">
            Las <strong>subconsultas correlacionadas</strong> hacen una subconsulta por cada fila (lento).
            Prefiere <strong>JOINs</strong> cuando sea posible.
          </InfoBox>
        </>
      )
    },
    {
      ...
      content: (
        <>
          <p><strong>GROUP BY</strong> agrupa <strong>registros</strong>. <strong>HAVING</strong> filtra grupos (como <strong>WHERE</strong> pero para agregaciones):</p>
          <CodeBlock
            language="sql"
...
HAVING SUM(p.total) > 1000;`}
      ...
      content: (
        <>
          <Table
            ...
            rows={[
              ['<strong>COUNT(*)</strong>', 'Cuenta todas las filas', 'SELECT COUNT(*) FROM usuarios'],
              ['<strong>COUNT(columna)</strong>', 'Cuenta no nulos', 'SELECT COUNT(email) FROM usuarios'],
              ['<strong>SUM(columna)</strong>', 'Suma valores numéricos', 'SELECT SUM(total) FROM pedidos'],
              ['<strong>AVG(columna)</strong>', 'Promedio de valores', 'SELECT AVG(edad) FROM usuarios'],
              ['<strong>MAX(columna)</strong>', 'Valor máximo', 'SELECT MAX(precio) FROM productos'],
              ['<strong>MIN(columna)</strong>', 'Valor mínimo', 'SELECT MIN(precio) FROM productos'],
              ['<strong>GROUP_CONCAT()</strong>', 'Concatena valores', 'SELECT GROUP_CONCAT(nombre) FROM usuarios']
            ]}
          />
        </>
      )
    },
    {
      ...
      content: (
        <>
          <p>Las <strong>vistas</strong> son consultas guardadas que funcionan como <strong>tablas virtuales</strong>:</p>
          <CodeBlock
            language="sql"
...
DROP VIEW usuarios_activos;`}
          />
          <InfoBox type="tip">
            Las <strong>vistas</strong> simplifican consultas complejas y reutilizables. Úsalas para mantener código limpio.
          </InfoBox>
        </>
      )
    },
    {
      ...
      content: (
        <>
          <p><strong>Window functions</strong> operan sobre un conjunto de filas relacionadas:</p>
          <CodeBlock
            language="sql"
            code={`-- Ranking
SELECT
